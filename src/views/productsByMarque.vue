...
export default {
    ...
    watch: {
        ...
        id: {
            ...
            handler(newId) {
                this.productMarques(newId); // Recharger les produits lorsque l'ID change
            },
        },
    },

    async mounted() {
        await this.productMarques(this.id)
        await this.getMarques(this.id)
    },
    methods: {
        async search() {
            try {
        ...
        async productMarques(id) {
            try {
                this.products = [];
                this.isLoading = true
                const response = await fetch(`https://vente-gq1m.onrender.com/api/marque/${id}`, {
                    method: 'POST',
                    headers: {
